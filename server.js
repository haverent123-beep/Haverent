...
app.post(["/api/auth/register", "/api/register"], async (req,res)=>{
  ...
  try{
    ...
    const ownerToken = safeRole === "owner" ? `OWN-${crypto.randomBytes(5).toString("hex").toUpperCase()}` : undefined;
    const user=await User.create({name,email:email.toLowerCase(),password:hash,role:safeRole,verificationStatus,phone:String(req.body?.phone||"").trim(),providerServices:Array.isArray(req.body?.providerServices)?req.body.providerServices.slice(0,10):[],ownerToken});
    res.status(201).json({token:tokenFor(user),user:{id:user._id,name:user.name,email:user.email,role:user.role,verificationStatus:user.verificationStatus,phone:user.phone,providerServices:user.providerServices,ownerToken:user.ownerToken}});
...
app.post(["/api/auth/login", "/api/login"], async (req,res)=>{
  ...
  try{
    ...
    res.json({token:tokenFor(user),user:{id:user._id,name:user.name,email:user.email,role:user.role,verificationStatus:user.verificationStatus,phone:user.phone,providerServices:user.providerServices,ownerToken:user.ownerToken}});
  }catch(e){res.status(500).json({message:e.message});}
});
...
app.post("/api/payments/manual/submit", auth, async (req,res) => {
 try {
  ...
  if(!["property_upload","provider_registration","booking"].includes(purpose)) return res.status(400).json({message:"Invalid payment purpose"});
...
app.patch("/api/admin/providers/:id",adminAuth,async(req,res)=>{
  try{
    ...
    if(status==="verified"){ const paid=await Payment.findOne({user:req.params.id,purpose:"provider_registration",status:"verified"}); if(!paid) return res.status(400).json({message:"Verify the provider registration payment before approving this provider"}); }
    const provider=await User.findOneAndUpdate({ _id:req.params.id, role:"provider" },{verificationStatus:status},{new:true}).select("-password");
    if(!provider) return res.status(404).json({message:"Provider not found"});
    res.json({provider,message:`Provider ${status}`});
  }catch(e){res.status(500).json({message:e.message});}
});
...
app.patch("/api/admin/payments/:id",adminAuth,async(req,res)=>{
  try{
    ...
    if(status==="verified") payment.receiptNo=`PAY-${new Date().getFullYear()}-${crypto.randomBytes(4).toString("hex").toUpperCase()}`;
    await payment.save();
    if(payment.purpose==="booking" && payment.booking){
      const booking=await Booking.findById(payment.booking);
      if(booking){booking.paymentStatus=status;booking.paymentId=payment._id;booking.receiptNo=booking.receiptNo||payment.receiptNo;await booking.save();}
    }
    const populated=await Payment.findById(payment._id).populate("user","name email").populate("booking");
...
app.post("/api/bookings",auth,async(req,res)=>{
 try{
  ...
  await booking.populate("property"); res.status(201).json({booking,requiresPayment:true,amount:BOOKING_FEE,message:"Booking created. Complete UPI payment and submit the transaction ID for admin verification."});
 }catch(e){res.status(500).json({message:e.message});}
});
